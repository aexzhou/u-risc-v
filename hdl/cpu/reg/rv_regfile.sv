...
module rv_regfile # (
  parameter int DW = 32
) (rs1, rs2, writeR, write_data, write, clk, rst, data_out1, data_out2);
  input      [4:0]  rs1;        // Source register 1 index
  input      [4:0]  rs2;        // Source register 2 index
  input      [4:0]  writeR;
  input      [DW-1:0] write_data; //write data
  input             write, clk, rst; //enable for write
  output reg [DW-1:0] data_out1;
  output reg [DW-1:0] data_out2;
  wire [DW-1:0] write_data_protected;

  assign write_data_protected = (writeR == 5'd0)? DW'(0) : write_data; // Protects X0 to not be over-written by any value so X0 maintains having 64'd0.

  reg [DW-1:0] X [0:31]; // 32 X 64-bit registers

  // WRITING LOGIC
  always @(posedge clk) begin
    if(rst)begin
      X <= '{default:'0}; // Upon reset, set all reg data to 64'b0
    end else begin
      if(write) begin
