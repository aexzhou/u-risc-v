...
class test_asm_bgeu_not_taken extends test_asm_base;

    function new();
        super.new("test_asm_bgeu_not_taken");
        hex_file = "test_asm_bgeu_not_taken.hex";
    endfunction
    ...
    virtual task check();
        `ASSERT_EQ(`U_REGFILE_PATH.X[10], 64'd1)   // bgeu not taken unsigned (1 not >=u 0xFFFFFFFF)
    endtask
