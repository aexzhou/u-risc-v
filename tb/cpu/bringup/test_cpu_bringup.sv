...
module test_cpu_bringup_arithmetic;
    ...
    initial begin
        ...
        // addi x1, x0, 5
        env.u_cpu.u_ifu.u_imem.memory[0] = 32'b00000000010100000000000010010011;
        // addi x2, x0, 6
        env.u_cpu.u_ifu.u_imem.memory[1] = 32'b00000000011000000000000100010011;
        // addi x3, x1, 7
        env.u_cpu.u_ifu.u_imem.memory[2] = 32'b00000000011100001000000110010011;
        // add x4, x1, x2
        env.u_cpu.u_ifu.u_imem.memory[3] = 32'b00000000001000001000001000110011;
        // sub x4, x4, x2
        env.u_cpu.u_ifu.u_imem.memory[4] = 32'b01000000001000100000001000110011;

        env.apply_reset();
        env.run_cycles(20);

        if (env.u_cpu.u_idu.u_regfile.X[1] != 64'h5) pass = 1'b0;
        if (env.u_cpu.u_idu.u_regfile.X[2] != 64'h6) pass = 1'b0;
        if (env.u_cpu.u_idu.u_regfile.X[3] != 64'hc) pass = 1'b0;
        if (env.u_cpu.u_idu.u_regfile.X[4] != 64'h5) pass = 1'b0;

        $display("[arith] X1=0x%0h X2=0x%0h X3=0x%0h X4=0x%0h => %s",
                 env.u_cpu.u_idu.u_regfile.X[1],
                 env.u_cpu.u_idu.u_regfile.X[2],
                 env.u_cpu.u_idu.u_regfile.X[3],
...
module test_cpu_bringup_branch;
    ...
    initial begin
        ...
        env.u_cpu.u_ifu.u_imem.memory[2]  = 32'b00000010000100000000111001100011; // beq x0, x1, #60 (not taken)
        env.u_cpu.u_ifu.u_imem.memory[3]  = 32'b00000000001000001000001000110011; // add x4, x1, x2
        env.u_cpu.u_ifu.u_imem.memory[4]  = 32'b00000010001000001001111001100011; // bne x1, x2, #60 (taken)
        env.u_cpu.u_ifu.u_imem.memory[5]  = 32'b00000000010100000000000010010011; // flushed
        env.u_cpu.u_ifu.u_imem.memory[6]  = 32'b00000000011000000000000100010011; // flushed
        env.u_cpu.u_ifu.u_imem.memory[7]  = 32'b00000000001000001000001000110011; // flushed
        env.u_cpu.u_ifu.u_imem.memory[19] = 32'b00000000100000000000000010010011; // addi x1, x0, 8
        env.u_cpu.u_ifu.u_imem.memory[20] = 32'b00000000100100000000000100010011; // addi x2, x0, 9

        env.apply_reset();
        env.run_cycles(45);

        if (env.u_cpu.u_idu.u_regfile.X[1] != 64'h8) pass = 1'b0;
        if (env.u_cpu.u_idu.u_regfile.X[2] != 64'h9) pass = 1'b0;
        if (env.u_cpu.u_idu.u_regfile.X[4] != 64'hb) pass = 1'b0;
